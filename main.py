...
def main(demo=False):
    if args.sparsity_ratio != 0:
        ...
        if args.save_model:
            d2prune.model.save_pretrained(args.save_model)
        ...
        ## ppl test
        ppl_test = eval_ppl(args, d2prune.model, tokenizer)
        ...
        if args.eval_zero_shot:
            ...
            if demo:
                task_list = ['boolq']
            results = eval_zero_shot(args, d2prune.model, tokenizer, task_list=task_list)
            args.logger.info("\n" + make_table(results))
